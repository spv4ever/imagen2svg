...
3. Procesa el lote.
4. Cada PNG se vectoriza con pasada simple, corte de luminosidad 0.450, motas 2, suavizado 1.00 y optimización 0.200, generando elementos `<path>` dentro de un SVG.
...
La exportación de esta aplicación evita ese problema generando rutas vectoriales reales (`<path d="...">`) a partir de los contornos de la imagen, usando la configuración validada de Inkscape: pasada simple, modo Corte de luminosidad, umbral 0.450, Motas 2, Suavizar bordes 1.00 y Optimizar 0.200. Inkscape ya no se usa para convertir directamente el PNG a SVG, porque ese flujo puede producir un SVG con mapa de bits incrustado. Si `inkscape` está disponible en el `PATH`, se usa únicamente como postprocesador opcional para normalizar el SVG ya vectorizado a SVG plain.
